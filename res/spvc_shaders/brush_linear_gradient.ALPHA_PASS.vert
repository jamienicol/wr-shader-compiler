...
Gradient fetch_gradient(int address)
{
    ...
    return Gradient(data[0], int(data[1].x), data[1].yz);
}

void linear_gradient_brush_vs(VertexInfo vi, int prim_address, RectWithSize local_rect, RectWithSize segment_rect, ivec4 prim_user_data, int specific_resource_address, mat4 transform, PictureTask pic_task, int brush_flags, vec4 texel_rect)
{
    int param = prim_address;
    Gradient gradient = fetch_gradient(param);
    if ((brush_flags & 2) != 0)
    {
        vec2 _939 = (vi.local_pos - segment_rect.p0) / segment_rect.size;
        varying_vec4_0 = vec4(varying_vec4_0.x, varying_vec4_0.y, _939.x, _939.y);
        vec2 _952 = (varying_vec4_0.zw * (texel_rect.zw - texel_rect.xy)) + texel_rect.xy;
        varying_vec4_0 = vec4(varying_vec4_0.x, varying_vec4_0.y, _952.x, _952.y);
        vec2 _959 = varying_vec4_0.zw * local_rect.size;
        varying_vec4_0 = vec4(varying_vec4_0.x, varying_vec4_0.y, _959.x, _959.y);
    }
    else
    {
        vec2 _967 = vi.local_pos - local_rect.p0;
        varying_vec4_0 = vec4(varying_vec4_0.x, varying_vec4_0.y, _967.x, _967.y);
    }
    vec2 start_point = gradient.start_end_point.xy;
    vec2 end_point = gradient.start_end_point.zw;
    vec2 dir = end_point - start_point;
    flat_varying_vec4_0 = vec4(start_point.x, start_point.y, flat_varying_vec4_0.z, flat_varying_vec4_0.w);
    vec2 _991 = dir / vec2(dot(dir, dir));
    flat_varying_vec4_0 = vec4(flat_varying_vec4_0.x, flat_varying_vec4_0.y, _991.x, _991.y);
...
void main()
{
    ...
    linear_gradient_brush_vs(param_20, param_21, param_22, param_23, param_24, param_25, param_26, param_27, param_28, param_29);
}
