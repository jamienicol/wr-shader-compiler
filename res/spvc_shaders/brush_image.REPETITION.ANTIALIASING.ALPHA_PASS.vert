...
void image_brush_vs(VertexInfo vi, int prim_address, RectWithSize prim_rect, RectWithSize segment_rect, ivec4 prim_user_data, int specific_resource_address, mat4 transform, PictureTask pic_task, int brush_flags, vec4 segment_data)
{
    ...
    if ((brush_flags & 2) != 0)
    {
        ...
        if ((brush_flags & 128) != 0)
        {
            ...
            if ((brush_flags & 64) != 0)
            {
                ...
                bool _1180 = segment_uv_size.x < epsilon;
                bool _1188;
                if (!_1180)
                {
                    _1188 = stretch_size.x < epsilon;
                }
                else
                {
                    _1188 = _1180;
                ...
                bool _1212 = segment_uv_size.y < epsilon;
                bool _1220;
                if (!_1212)
                {
                    _1220 = stretch_size.y < epsilon;
                }
                else
                {
                    _1220 = _1212;
    ...
    vec2 _1416 = mix(uv0, uv1, f) - min_uv;
    varying_vec4_0 = vec4(varying_vec4_0.x, varying_vec4_0.y, _1416.x, _1416.y);
    vec2 _1422 = varying_vec4_0.zw / texture_size;
    varying_vec4_0 = vec4(varying_vec4_0.x, varying_vec4_0.y, _1422.x, _1422.y);
    vec2 _1428 = varying_vec4_0.zw * repeat;
    varying_vec4_0 = vec4(varying_vec4_0.x, varying_vec4_0.y, _1428.x, _1428.y);
    if (perspective_interpolate == 0.0)
    {
        vec2 _1439 = varying_vec4_0.zw * vi.world_pos.w;
        varying_vec4_0 = vec4(varying_vec4_0.x, varying_vec4_0.y, _1439.x, _1439.y);
    }
