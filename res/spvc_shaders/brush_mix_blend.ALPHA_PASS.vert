...
void mix_blend_brush_vs(VertexInfo vi, int prim_address, RectWithSize local_rect, RectWithSize segment_rect, ivec4 prim_user_data, int specific_resource_address, mat4 transform, PictureTask pic_task, int brush_flags, vec4 unused)
{
    vec2 device_pos = (vi.world_pos.xy * pic_task.device_pixel_scale) / vec2(max(0.0, vi.world_pos.w));
    vec2 texture_size = vec2(vec3(textureSize(sPrevPassColor, 0)).xy);
    flat_varying_ivec4_0.x = prim_user_data.x;
    int param = prim_user_data.z;
    PictureTask src_task = fetch_picture_task(param);
    vec2 src_uv = (device_pos + src_task.common_data.task_rect.p0) - src_task.content_origin;
    vec2 _989 = src_uv / texture_size;
    varying_vec4_0 = vec4(_989.x, _989.y, varying_vec4_0.z, varying_vec4_0.w);
    varying_vec4_0.w = src_task.common_data.texture_layer_index;
    int param_1 = prim_user_data.y;
    RenderTaskCommonData backdrop_task = fetch_render_task_common_data(param_1);
    vec2 backdrop_uv = (device_pos + backdrop_task.task_rect.p0) - src_task.content_origin;
    vec2 _1012 = backdrop_uv / texture_size;
    varying_vec4_1 = vec4(_1012.x, _1012.y, varying_vec4_1.z, varying_vec4_1.w);
    varying_vec4_1.w = backdrop_task.texture_layer_index;
}
...
void main()
{
    ...
    if (transform.is_axis_aligned)
    {
        ...
        VertexInfo _874 = write_vertex(param_5, param_6, param_7, param_8, param_9);
        vi = _874;
    ...
    else
    {
        ...
        VertexInfo _909 = write_transform_vertex(param_11, param_12, param_13, param_14, param_15, param_16, param_17);
        vi = _909;
    }
    ...
    mix_blend_brush_vs(param_20, param_21, param_22, param_23, param_24, param_25, param_26, param_27, param_28, param_29);
}
