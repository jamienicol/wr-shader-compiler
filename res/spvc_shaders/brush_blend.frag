...
highp vec3 SrgbToLinear(highp vec3 color)
{
    ...
    bvec3 _269 = lessThanEqual(color, vec3(0.040449999272823333740234375));
    return vec3(_269.x ? c1.x : c2.x, _269.y ? c1.y : c2.y, _269.z ? c1.z : c2.z);
}
...
highp vec3 LinearToSrgb(highp vec3 color)
{
    ...
    bvec3 _291 = lessThanEqual(color, vec3(0.003130800090730190277099609375));
    return vec3(_291.x ? c1.x : c2.x, _291.y ? c1.y : c2.y, _291.z ? c1.z : c2.z);
}
...
Fragment blend_brush_fs()
{
    ...
    if (alpha != 0.0)
    {
        _441 = Cs.xyz / vec3(alpha);
    }
    else
    {
        _441 = Cs.xyz;
    }
    highp vec3 color = _441;
    switch (flat_varying_ivec4_0.x)
    {
        ...
        case 11:
        {
            ...
            highp vec4 _502 = ComponentTransfer(param_8);
            colora = _502;
            color = colora.xyz;
            alpha = colora.w;
            break;
