...
void image_brush_vs(VertexInfo vi, int prim_address, RectWithSize prim_rect, RectWithSize segment_rect, ivec4 prim_user_data, int specific_resource_address, mat4 transform, PictureTask pic_task, int brush_flags, vec4 segment_data)
{
    ...
    if ((brush_flags & 2) != 0)
    {
        ...
        if ((brush_flags & 128) != 0)
        {
            ...
            if ((brush_flags & 64) != 0)
            {
                ...
                bool _1022 = segment_uv_size.x < epsilon;
                bool _1030;
                if (!_1022)
                {
                    _1030 = stretch_size.x < epsilon;
                }
                else
                {
                    _1030 = _1022;
                ...
                bool _1054 = segment_uv_size.y < epsilon;
                bool _1062;
                if (!_1054)
                {
                    _1062 = stretch_size.y < epsilon;
                }
                else
                {
                    _1062 = _1054;
    ...
    vec2 _1228 = mix(uv0, uv1, f) - min_uv;
    varying_vec4_0 = vec4(varying_vec4_0.x, varying_vec4_0.y, _1228.x, _1228.y);
    vec2 _1234 = varying_vec4_0.zw / texture_size;
    varying_vec4_0 = vec4(varying_vec4_0.x, varying_vec4_0.y, _1234.x, _1234.y);
    vec2 _1240 = varying_vec4_0.zw * repeat;
    varying_vec4_0 = vec4(varying_vec4_0.x, varying_vec4_0.y, _1240.x, _1240.y);
    if (perspective_interpolate == 0.0)
    {
        vec2 _1251 = varying_vec4_0.zw * vi.world_pos.w;
        varying_vec4_0 = vec4(varying_vec4_0.x, varying_vec4_0.y, _1251.x, _1251.y);
    }
