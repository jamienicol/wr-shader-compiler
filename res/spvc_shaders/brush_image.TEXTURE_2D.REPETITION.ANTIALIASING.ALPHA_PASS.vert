...
void image_brush_vs(VertexInfo vi, int prim_address, RectWithSize prim_rect, RectWithSize segment_rect, ivec4 prim_user_data, int specific_resource_address, mat4 transform, PictureTask pic_task, int brush_flags, vec4 segment_data)
{
    ...
    if ((brush_flags & 2) != 0)
    {
        ...
        if ((brush_flags & 128) != 0)
        {
            ...
            if ((brush_flags & 64) != 0)
            {
                ...
                bool _1173 = segment_uv_size.x < epsilon;
                bool _1181;
                if (!_1173)
                {
                    _1181 = stretch_size.x < epsilon;
                }
                else
                {
                    _1181 = _1173;
                ...
                bool _1205 = segment_uv_size.y < epsilon;
                bool _1213;
                if (!_1205)
                {
                    _1213 = stretch_size.y < epsilon;
                }
                else
                {
                    _1213 = _1205;
    ...
    vec2 _1409 = mix(uv0, uv1, f) - min_uv;
    varying_vec4_0 = vec4(varying_vec4_0.x, varying_vec4_0.y, _1409.x, _1409.y);
    vec2 _1415 = varying_vec4_0.zw / texture_size;
    varying_vec4_0 = vec4(varying_vec4_0.x, varying_vec4_0.y, _1415.x, _1415.y);
    vec2 _1421 = varying_vec4_0.zw * repeat;
    varying_vec4_0 = vec4(varying_vec4_0.x, varying_vec4_0.y, _1421.x, _1421.y);
    if (perspective_interpolate == 0.0)
    {
        vec2 _1432 = varying_vec4_0.zw * vi.world_pos.w;
        varying_vec4_0 = vec4(varying_vec4_0.x, varying_vec4_0.y, _1432.x, _1432.y);
    }
