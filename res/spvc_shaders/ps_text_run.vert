...
Glyph fetch_glyph(int specific_prim_address, int glyph_index)
{
    ...
    bvec2 _526 = bvec2((uint(glyph_index) % 2u) != 0u);
    vec2 glyph = vec2(_526.x ? data.zw.x : data.xy.x, _526.y ? data.zw.y : data.xy.y);
    return Glyph(glyph);
}
...
void main()
{
    ...
    RectWithSize glyph_rect = RectWithSize(((res.offset + raster_glyph_offset) * glyph_scale_inv) + text_offset, (res.uv_rect.zw - res.uv_rect.xy) * glyph_scale_inv);
    vec2 local_pos = glyph_rect.p0 + (glyph_rect.size * aPosition.xy);
    vec2 param_8 = local_pos;
    RectWithSize param_9 = ph.local_clip_rect;
    local_pos = clamp_rect(param_8, param_9);
    ...
    vec4 param_10 = world_pos;
    ClipArea param_11 = clip_area;
    write_clip(param_10, param_11);
    switch (color_mode)
    {
        case 1:
        case 7:
    ...
    vec2 _847 = mix(st0, st1, f);
    varying_vec4_0 = vec4(_847.x, _847.y, varying_vec4_0.z, varying_vec4_0.w);
