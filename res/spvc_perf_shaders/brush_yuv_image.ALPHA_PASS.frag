...
void main()
{
    ...
    if (vFormat == 1)
    {
        highp vec3 _695 = _704;
        _695.x = texture(sColor0, vec3(clamp(vUv_Y.xy, vUvBounds_Y.xy, vUvBounds_Y.zw), vUv_Y.z)).x;
        highp vec3 _697 = _695;
        _697.y = texture(sColor1, vec3(clamp(vUv_U.xy, vUvBounds_U.xy, vUvBounds_U.zw), vUv_U.z)).x;
        highp vec3 _699 = _697;
        _699.z = texture(sColor2, vec3(clamp(vUv_V.xy, vUvBounds_V.xy, vUvBounds_V.zw), vUv_V.z)).x;
        _705 = _699;
    }
    else
    {
        ...
        if (vFormat == 0)
        {
            highp vec3 _701 = _712;
            _701.x = texture(sColor0, vec3(clamp(vUv_Y.xy, vUvBounds_Y.xy, vUvBounds_Y.zw), vUv_Y.z)).x;
            highp vec4 _522 = texture(sColor1, vec3(clamp(vUv_U.xy, vUvBounds_U.xy, vUvBounds_U.zw), vUv_U.z));
            _706 = vec3(_701.x, _522.x, _522.y);
        }
        else
        {
            highp vec3 _707;
            if (vFormat == 2)
            {
                _707 = texture(sColor0, vec3(clamp(vUv_Y.xy, vUvBounds_Y.xy, vUvBounds_Y.zw), vUv_Y.z)).yzx;
            }
            else
            {
                _707 = vec3(0.0);
    ...
    switch (0u)
    {
        default:
        {
            ...
            _708 = 0.5 + (_617 * (((0.8431026935577392578125 * _617) * _617) - 1.14453601837158203125));
            break;
        }
    }
    highp float _710;
    switch (0u)
    {
        default:
        {
            ...
            _710 = texelFetch(sPrevPassAlpha, ivec3(ivec2(_656), int(vClipMaskUv.z + 0.5)), 0).x;
            break;
        }
    }
    oFragColor = (vec4(vYuvColorMatrix * ((_705 * vCoefficient) - vec3(0.06274999678134918212890625, 0.501959979534149169921875, 0.501959979534149169921875)), 1.0) * _708) * _710;
}
