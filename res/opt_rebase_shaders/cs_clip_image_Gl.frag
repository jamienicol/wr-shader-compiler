...
void main ()
{
  ...
  vec2 tmpvar_8;
  tmpvar_8 = min (max (vClipMaskImageUv, vec2(0.0, 0.0)), vLocalPos.ww);
  if ((tmpvar_8 != vClipMaskImageUv)) {
    discard;
  };
  vec3 tmpvar_9;
  tmpvar_9.xy = min (max ((
    ((tmpvar_8 / vLocalPos.w) * vClipMaskUvRect.zw)
   + vClipMaskUvRect.xy), vClipMaskUvInnerRect.xy), vClipMaskUvInnerRect.zw);
  tmpvar_9.z = vLayer;
  vec4 tmpvar_10;
