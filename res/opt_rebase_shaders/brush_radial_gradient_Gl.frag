...
void main ()
{
  ...
  int tmpvar_13;
  tmpvar_13 = (flat_varying_highp_int_address_0 + min (max (
    (2 * int(floor(x_12)))
  , 0), 258));
  ivec2 tmpvar_14;
  tmpvar_14.x = int((uint(tmpvar_13) % 1024u));
  tmpvar_14.y = int((uint(tmpvar_13) / 1024u));
  oFragColor = mix (texelFetchOffset (sGpuCache, tmpvar_14, 0, ivec2(0, 0)), texelFetchOffset (sGpuCache, tmpvar_14, 0, ivec2(1, 0)), fract(x_12));
}
